...
def birthday_filter(birthday):
    ...
    '''new_minite = str(dt.minute)
    if dt.minite < 10:
        new_minute = '0' + str(dt.minute)

    :return u'[发布于 %s年%s月%s日 %s:%s  ' % (dt.year, dt.mouth, dt.day, dt.hour, new_minute)
    '''
    b_year = birthday[:4]
    b_mouth = birthday[4:6]
    b_day = birthday[6:]

    age = int(dt.year) - int(b_year) - 1

    if int(b_mouth) > (int(dt.month) - 1):
...
class User(AbstractUser):
    ...
    """
    name = models.CharField(max_length=20, default='沈峰')  # 姓名
    sex = models.CharField(max_length=3, default='男')      # 性别
    birthday = models.CharField(max_length=20, default='1994-02-17')  # 八位数字，年龄用生日计算
    ...
    job_time = models.CharField(max_length=20, default='2017-06-01')  # 6位数字， 获得职称的时间

    # 本次新增
    xue_li = models.CharField(max_length=30, default='本科')  # 学历
    school = models.CharField(max_length=30, default='江苏师范大学')  # 学校
    graduate_time = models.CharField(max_length=20, default='2016-06-20')  # 毕业时间
    job_join_time = models.CharField(max_length=20, default='2017-06-01')  # 入职时间
    team_belong = models.CharField(max_length=20, default='C640')  # 所属部门

    phone = models.CharField(max_length=20, default='18361296750')  # 手机号
